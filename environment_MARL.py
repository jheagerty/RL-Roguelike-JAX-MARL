...
class RL_Roguelike_JAX_MARL(MultiAgentEnv):
    ...
    def function_mapper(self, key: chex.PRNGKey, state: GameState, action: int, unit, target):
        """Updated function mapper to handle both base actions and multiple ability slots"""
        def do_base_action(key: chex.PRNGKey):
            action_fns = [lambda s, u, t: action.execute(key, s, u, t) for action in base_action_functions]
        ...
        def do_ability_action(key: chex.PRNGKey):
            def execute_ability(i, args):
                ...
                # Now passing ability_idx to execute
                return ability_action_functions[i].execute(key, state, unit, target, ability_idx)
            
            # Get ability slot index (0 for ability_state_1, 1 for ability_state_2, etc)
            ability_slot = action - num_actions
            
            # Get ability index using switch statement for each slot
            def get_ability_idx(slot):
                return lax.switch(slot,
                    [
                        lambda: unit.ability_state_1.ability_index,
                        # lambda: unit.ability_state_2.ability_index,
                        # lambda: unit.ability_state_3.ability_index,
                    ]
                )
            
            ability_idx = get_ability_idx(ability_slot)
