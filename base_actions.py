...
class MoveAction:
    ...
    def is_valid(self, state, unit, target):
        ...
        enough_movement_points = unit.movement_points_current >= jnp.sqrt(self.dx**2 + self.dy**2)
        return jnp.logical_and(valid_move, enough_movement_points)

    def execute(self, state, unit, target):
        def do_move(_):
            new_x, new_y = unit.location_x + self.dx, unit.location_y + self.dy
            distance = jnp.sqrt(self.dx**2 + self.dy**2)
            unit_movement_points = unit.movement_points_current - distance
            def distance_to_enemy_from_player():
                return euclidean_distance(new_x, new_y, state.enemy.location_x, state.enemy.location_y)
            def distance_to_player_from_enemy():
                return euclidean_distance(state.player.location_x, state.player.location_y, new_x, new_y)
        ...
        def invalid_move(_):
            # debug.print("invalid move: {move}, unit: {unit}", move=(self.dx, self.dy), unit = unit.unit_id)
            return do_invalid_move(state, unit, target)

        result_state = lax.cond( # TODO USE LAX SWITCH ONCE MORE UNITS
...
class MeleeAttackAction:
    ...
    def execute(self, state, unit, target):
        def do_melee_attack(_):
            target_health_after = jnp.maximum(0, target.health_current - unit.melee_attack_base_damage)
...
class RangedAttackAction:
    ...
    def execute(self, state, unit, target):
        def do_ranged_attack(_):
            target_health_after = jnp.maximum(0, state.enemy.health_current - unit.ranged_attack_base_damage)
            unit_action_points = unit.action_points_current - 1
            new_target = target.replace(health_current=jnp.float32(target_health_after))
            new_unit = unit.replace(action_points_current=jnp.float32(unit_action_points))
